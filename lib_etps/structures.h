...
inline double CalcAppEnergy(std::vector<double> sumR, std::vector<double> sumG, std::vector<double> sumB,
                            std::vector<double> sumR2, std::vector<double> sumG2, std::vector<double> sumB2, int size, int numP, int imageDimension, int pNorm){
    std::vector<double> appEnergy(imageDimension, 0.0);
    double result=0;

    for(int d=0; d<imageDimension; ++d){
        appEnergy.at(d) = CalcAppEnergy(sumR.at(d), sumG.at(d), sumB.at(d),
                    sumR2.at(d), sumG2.at(d), sumB2.at(d), size, numP);
         result += std::pow( std::fabs(appEnergy.at(d)) , pNorm);
    }
    result = std::pow(result, (1.0/pNorm));
...
struct PixelData {
    ...
    std::vector<double> sumR2, sumG2, sumB2;
...
struct Pixel { // : public custom_alloc {
    ...
    void CalcRGBDSum(const std::vector<cv::Mat>& img, std::vector<double>& sumR, std::vector<double>& sumR2, std::vector<double>& sumG, std::vector<double>& sumG2,
        std::vector<double>& sumB, std::vector<double>& sumB2) const
    {
        int dim = img.size();

        sumR = std::vector<double>(dim, 0.0); sumR2 = std::vector<double>(dim, 0.0);
        sumG = std::vector<double>(dim, 0.0); sumG2 = std::vector<double>(dim, 0.0);
        sumB = std::vector<double>(dim, 0.0); sumB2 = std::vector<double>(dim, 0.0);

        for (int d=0; d<dim; ++d) {
            CalcRGBSum(img.at(d), sumR.at(d), sumR2.at(d), sumG.at(d), sumG2.at(d), sumB.at(d), sumB2.at(d));
        }
    }
    ...
    */
    ...
    inline void CalcPixelData(const std::vector<cv::Mat>& img, PixelData& pd)
    {
        pd.p = this;
        CalcRowColSum(pd.sumRow, pd.sumRow2, pd.sumCol, pd.sumCol2, pd.sumRowCol);
        CalcRGBDSum(img, pd.sumR, pd.sumR2, pd.sumG, pd.sumG2, pd.sumB, pd.sumB2);
        pd.size = GetSize();
        pd.imageDimension = img.size();
    }
    ...
    inline void CalcPixelDataStereo(const std::vector<cv::Mat>& img, const cv::Mat1d& imgDisp,
        Plane_d& planeP, Plane_d& planeQ, double inlierThresh, double noDisp, PixelData& pd)
    {
        CalcPixelData(img, pd);
        pd.sumDispP = CalcDispSum(imgDisp, planeP, inlierThresh, noDisp);
    ...
    void Split(int row1, int row2, int col1, int col2, Pixel& p11, Pixel& p12, Pixel& p21, Pixel& p22);
    void SplitRow(int row1, int row2, int col, Pixel& p11, Pixel& p21);
    void SplitColumn(int row, int col1, int col2, Pixel& p11, Pixel& p12);
    void CopyTo(int row, int col, Pixel& p11);
...
public:
    ...
    std::vector<double> sumR, sumG, sumB;    // sum of colors
    std::vector<double> sumR2, sumG2, sumB2; // sum of squares of colors
    double eApp;
    double eReg;
    ...
    int imageDimensions;
public:
    Superpixel(int _id, int _imageDimensions, int _pNorm) : id(_id), borderLength(0), sumRow(0), sumCol(0), sumRowCol(0),
            sumRow2(0), sumCol2(0), sumR(_imageDimensions,0.0), sumG(_imageDimensions,0.0), sumB(_imageDimensions,0.0),
            sumR2(_imageDimensions,0.0), sumG2(_imageDimensions,0.0), sumB2(_imageDimensions,0.0),
            eApp(0), eReg(0), size(0), initialSize(0), numP(0), pNorm(_pNorm), imageDimensions(_imageDimensions){ }
    virtual ~Superpixel() { }
    ...
    virtual void AddPixelInit(PixelData& pd) 
    {
        ...
        for(int d=0; d<imageDimensions; ++d){
            sumR.at(d) += pd.sumR.at(d);
            sumG.at(d)  += pd.sumG.at(d);
            sumB.at(d)  += pd.sumB.at(d);
            sumR2.at(d)  += pd.sumR2.at(d);
            sumG2.at(d)  += pd.sumG2.at(d);
            sumB2.at(d)  += pd.sumB2.at(d);
        }
        size += pd.size;
        numP++;
    ...
    {
        AddPixelInit(pd);
        eApp = CalcAppEnergy(sumR, sumG, sumB, sumR2, sumG2, sumB2, size, numP, imageDimensions, pNorm);
    ...
    */
    ...
    void GetMeanColors(std::vector<double>& r, std::vector<double>& g, std::vector<double>& b) const
    {
        r = std::vector<double>(imageDimensions,0.0);
        g = std::vector<double>(imageDimensions,0.0);
        b = std::vector<double>(imageDimensions,0.0);

        for(int d=0; d<imageDimensions; ++d){
            r.at(d) = sumR.at(d) / size;
            g.at(d) = sumG.at(d) / size;
            b.at(d) = sumB.at(d) / size;
        }
    }
...
public:
    ...
    SuperpixelStereo(int _id, int _imageDimensions, int _pNorm) : Superpixel(_id,_imageDimensions, _pNorm), sumDisp(0), sumIRow(0), sumICol(0),
