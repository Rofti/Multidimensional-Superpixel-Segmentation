...
SPSegmentationEngine::SPSegmentationEngine(SPSegmentationParameters params, std::vector<Mat> im, Mat depthIm) :
    params(params)
{
    origImg = im;
...
void SPSegmentationEngine::Initialize(Superpixel* spGenerator(int, int, int))
{
    int img_rows = img.front().rows, img_cols = img.front().cols;
    int imageSize = img_rows * img_cols;
...
//    int gridSize = (int)sqrt((double)imageSize / params.superpixelNum);
    ...
    int imgSPixelsRows = iCeilDiv(img_rows, gridSize);
    int imgSPixelsCols = iCeilDiv(img_cols, gridSize);

    int imgPixelsRows = initDiv * (img_rows / gridSize) + iCeilDiv(img_rows % gridSize, maxPixelSize);
    int imgPixelsCols = initDiv * (img_cols / gridSize) + iCeilDiv(img_cols % gridSize, maxPixelSize);
    ...
    while (ri < initDiv * (img_rows / gridSize)) {
        for (int i = 0; i < maxN; i++) rowDims[ri++] = maxPS;
        for (int i = 0; i < minN; i++) rowDims[ri++] = minPS;
    }
    while (ci < initDiv * (img_cols / gridSize)) {
        for (int i = 0; i < maxN; i++) colDims[ci++] = maxPS;
        for (int i = 0; i < minN; i++) colDims[ci++] = minPS;
    }
    if (img_rows % gridSize > 0) {
        calcPixelSizes(img_rows % gridSize, maxPixelSize, maxPS, minPS, maxN, minN);
        for (int i = 0; i < maxN; i++) rowDims[ri++] = maxPS;
        for (int i = 0; i < minN; i++) rowDims[ri++] = minPS;
        rowSDims.back() = maxN + minN;
    }
    if (img_cols % gridSize > 0) {
        calcPixelSizes(img_cols % gridSize, maxPixelSize, maxPS, minPS, maxN, minN);
        for (int i = 0; i < maxN; i++) colDims[ci++] = maxPS;
        for (int i = 0; i < minN; i++) colDims[ci++] = minPS;
    ...
    for (int pi = 0; pi < imgSPixelsRows; pi++) {
        ...
        for (int pj = 0; pj < imgSPixelsCols; pj++) {
            ...
            Superpixel* sp = spGenerator(superPixelIdCount++,params.imageDimensions, params.pNorm); // Superpixel();
...
{
    Initialize([](int id, int imageDimensions, int pNorm) -> Superpixel* { return new SuperpixelStereo(id,imageDimensions,pNorm); });
    InitializePPImage();
    EstimatePlaneParameters();
    InitializeStereoEnergies();

    double gridSize = sqrt((double)img.front().rows*img.front().cols / superpixels.size());
...
{
    ppImg = Matrix<Pixel*>(img.front().rows, img.front().cols);
    UpdatePPImage();
}
...
void SPSegmentationEngine::ProcessImage()
{
    ...
    Initialize([](int id, int imageDimensions, int pNorm) { return new Superpixel(id, imageDimensions, pNorm); });
    ...
    for (int i = 0; i < pixelsImg.rows; i++) {
        ...
        for (int j = 0; j < pixelsImg.cols; j++) {
            ...
            if (pRowSize == 1 && pColSize == 1) {
                ...
                p.Split( newRow, newRow + 1, newCol, newCol + 1, p11, p12, p21, p22);
            }
...
{
    Mat result = origImg.front().clone();
